...
import { tPerson, tPPPar, tPPEq, tParData, tEqData, tHistory } from '../../const/variables.components';
...
export class AdvancePage {
    //#region Vars
		//#region Arrays
			ppArr: 	  tHistory = { time_: 0, data_: new Array(0) };
	...
	//#region pplFNs
		async addPer() {
			let userpop = await this.alertCtrl.create({
				...
				buttons: [ { text: 'Cancel' }, { text: 'Add', role: 'add', handler: data => { this.ppArr.data_.push({ name_: data.nameIN, data: new Array(0) } ); this.saveData(); } } ]
			}); userpop.present();
		}
		async openPer(id: number) {
			let addpop = await this.alertCtrl.create({
				...
				buttons: [ { text: 'Cancel' }, { text: 'Add', role: 'add', handler: data => { this.ppArr.data_[id].data.push({name: data.itemIN, count: 0, price: data.precioIN}); this.saveData(); } } ]
			}); addpop.present();
		}
		async viewPer(id: number) {
			...
			if(this.ppArr[id].data.length > 0) {
				...
				let i = 0; for(let item_ of this.ppArr.data_[id].data) { viewpop.addInput({ type: 'checkbox', label: item_.name + ' $' + item_.price, value: i.toString(), checked: false}); i++; }
				viewpop.addButton({ 
		...
		rmPer(id: number) { this.ppArr.data_.splice(id, 1); this.saveData(); }
		notMe(id: number) { if(id != 0) { return false; } return true; }
	//#endregion
	calculate(iRange: number, iSelec: number) { if(this.itsComplete(iSelec) < 1) { let m_data: any = this.calc.advanceCalc(iRange, iSelec);  this.ppEqARR = m_data.ppEq; this.ppParARR = m_data.ppPar } }
	...
	//#region cacheFNs
		readData() { 
			//#region Var
				let temp_data = this.glbVar.readCache();
				let sum = (temp_data.time_ + 21600);
				let actual_time = ((new Date()).getTime());
			//#endregion
			if(actual_time > sum) { this.ppArr = temp_data; }
			else { this.clearData(); }
		}
		saveData() { if(this.ppArr[0].data_.length > 0 || this.ppArr.data_.length > 1) { this.glbVar.writeCache(this.ppArr); } }
		clearData() {
			this.ppArr = { time_: (new Date()).getTime(), data_: new Array(0) };
			this.ppArr.data_.push({ name_: 'Me', data: new Array(0) }),
